...
class HiRAGConfig:
    ...
    graph_operations: GraphOperationsConfig = field(default_factory=GraphOperationsConfig)
...
class ConfigManager:
    ...
    def _create_config_from_dict(self, data: Dict[str, Any]) -> HiRAGConfig:
        ...
        if 'hirag' in data:
            ...
            if 'enable_hierachical_mode' in hirag_data:
                config.system_behavior.enable_hierarchical_mode = hirag_data['enable_hierachical_mode']
            if 'embedding_batch_num' in hirag_data:
                config.performance.embedding_batch_num = hirag_data['embedding_batch_num']
            if 'embedding_func_max_async' in hirag_data:
                config.performance.embedding_func_max_async = hirag_data['embedding_func_max_async']
    ...
    def _validate_config(self):
        ...
        if self.config.entity_processing.lexical_similarity_threshold < 0 or \
           self.config.entity_processing.lexical_similarity_threshold > 1:
